resource "aws_autoscaling_group" "T4Gapp1_asg" {
  ...
  force_delete               = true
  target_group_arns          = [aws_lb_target_group.T4Gapp1_tg.arn]

  launch_template {
    id      = aws_launch_template.T4Gapp1_LT.id
    version = "$Latest"
  }
...
resource "aws_autoscaling_attachment" "T4Gapp1_asg_attachment" {
  autoscaling_group_name = aws_autoscaling_group.T4Gapp1_asg.name
  alb_target_group_arn   = aws_lb_target_group.T4Gapp1_tg.arn
}
